...
export default function Card({ card, parent, index }) {
  ...
  if (card.type === 'E') {
    ...
    for (let i = 0; i < (index + 1); i++) { weight = weight + (i + 1) }
    for (let i = 0; i < len; i++) { sum = sum + (i + 1) }
        ...
        <div className="grid-middle-noGutter">
          ...
          <div className="col-12">
            <div className={"feature_storage-bar"}>
              ...
              {parent.cards.map((card, j) => (
                <div
                  key={card.title}
                  className="storage-bar-tier"
                  style={{ flexGrow: (j + 1) }}>
                  <span
                    className={ clsx("storage-bar-tier-label", index < j ? "display": "")}>
                    {card.title}
                  </span>
                </div>
              ))}
          ...
          <div className="col-7">
            <div className={"card_panel-right"}>
              {card.description && <div className="description" dangerouslySetInnerHTML={{ __html: card.description }}></div>}
            </div>
          </div>
